...
export default function InsurerHome({ goto }: { goto?: (r: string) => void }) {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      const [prof, players, matches] = await Promise.all([
        supabase.from('crm_insurer_profile').select('*').eq('insurer_id', uid).maybeSingle(),
  ...
  function openAthlete(id: number, route = 'insurance') {
    setAthleteId(id)
    goto?.(route)
  }
    ...
    <div className="grid" style={{ gap: 20 }}>
      ...
      <div style={{ position: 'relative', overflow: 'hidden', borderRadius: 18,
        ...
        <div className="flex gap" style={{ alignItems: 'center', gap: 15, flexWrap: 'wrap' }}>
          ...
          {goto && (
            <button className="btn btn-sm" onClick={() => goto('insurer-profile')}>

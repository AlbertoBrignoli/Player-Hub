// Sync server-side delle foto InTime nel Player Hub, pensato per girare
// da pg_cron ~1h dopo ogni partita dell'Olympiacos (vedi job
// intime-sync-postmatch): login su intime.gr, ricerca keyword, scarica le
// foto RECENTI (default ultimi 4 giorni) non ancora ingerite (registro
// public.intime_synced, che sopravvive alla pulizia mensile), le carica in
// storage e crea la riga crm_media nella cartella partita derivata dalla
// didascalia. L'archivio locale sul Mac resta la copia integrale storica.
...
async function secretsMap(): Promise<Record<string, string>> {
  const { data } = await supa.from("cp_secrets").select("key,value")
    .in("key", ["intime_secret", "intime_user", "intime_pass", "intime_keyword"]);
...
// ---------- cartella dalla didascalia (allineata allo script locale) ----------
...
Deno.serve(async (req0: Request) => {
  ...
  const limit = Number(body.limit) > 0 ? Math.min(Number(body.limit), 40) : 20;
  const days = Number(body.days) > 0 ? Number(body.days) : 4;
  const rawKeyword = body.keyword || secrets.intime_keyword || "ΠΙΡΟΛΑ";
  ...
  // ricerca: por.php registra i criteri in sessione (302), po.php renderizza
  const keywordEnc = encodeIso88597(normalizeKeyword(rawKeyword));
  ...
  // raccoglie solo le pagine con foto recenti (ordinate dalla piu' nuova)
  ...
  const recent = [...seen.values()].filter((p) => shotAtMs(p) >= cutoffMs);

  // esclude i codici gia' ingeriti (registro permanente)
  const codes = recent.map((p) => p.code);
  const synced = new Set<string>();
  for (let i = 0; i < codes.length; i += 200) {
    const { data } = await supa.from("intime_synced").select("code").in("code", codes.slice(i, i + 200));
  ...
  for (const p of batch) {
    if (Date.now() - startedAt > 100_000) break; // margine sul wall clock
    try {
      ...
      await supa.from("intime_synced").insert({ code: p.code });
      ingested++;
    } catch (e) {
  ...
  return json({
    ...
    ingested,
    pendingNext: fresh.length - ingested - errors.length,
    errors,
  });
});
